...
class __MainScreen__(tk.Frame):
	def __init__(self, control, __round, master=None):
		...
		if len(self.control.contestants) < self.control.contestantCount:
			self.control.contestants += self.contestants

		# copy info from contestants_info array (prefinal rounds only)
		if (self.roundNumber < 4 and len(self.control.contestants_info)):
			m = 0 + ((self.roundNumber - 1)*6)
			n = 6 + ((self.roundNumber - 1)*6)
			for i in range(m, n):
				#self.control.contestants[i].setName(self.control.contestants_info[i].getName())
				#self.control.contestants[i].setSchool(self.control.contestants_info[i].getSchool())
				self.control.contestants[i].setKey(self.control.contestants_info[i].getKey())
				self.control.contestants[i].setSound(self.control.contestants_info[i].getSound())

		self.control.contestants[0].setKey("i") # contestant 1
		self.control.contestants[1].setKey("]") # contestant 2
		self.control.contestants[2].setKey("k") # contestant 3
		self.control.contestants[3].setKey("p") # contestant 4
		self.control.contestants[4].setKey("[") # contestant 5
		self.control.contestants[5].setKey("u") # contestant 6

		# contestant keys: ABCDEF
		for i in range(self.contestantCount):
		...
		# bottom frame
		self.menuButton = tk.Button(self.frames[3], text='Menu', font=self.button_font, command=lambda:GamePauseScreen(control), anchor=tk.E, justify=tk.LEFT, bg=dark, fg=dark_txt, highlightbackground='#93A1A1', highlightcolor='#002B36')
		...
		self.circleButton7 = CustomButton(self.frames[4], 40, 40, "next", light, dark_txt, self.increment)
	...
	def correct(self, event):
		if (len(self.queue)):
			...
			# 	sys.stderr.write('(#'+str(h[0].getNum())+","+str(h[1])+")")
			# 	if self.history.index(h) != len(self.history)-1: sys.stderr.write(" ")
			# sys.stderr.write(']\n')
			# _debug("Pointer:", self.history_ptr, "Length:", len(self.history))
			self.queue.clear()
	...
	def increment(self):
		...
		if self.questionNumber < self.round.questionCount:
			...
			self.setTimer()
			self.questionNumber_label.config(text="Question " + str(self.questionNumber) + "/" + str(self.round.questionCount))
	...
	def load(self):
		...
		# middle frame
		self.frames[2].grid(row=1, column=0, rowspan=8, columnspan=2, padx=50, pady=55, sticky=tk.W+tk.E)
		# bottom frame 1/2
		self.frames[3].grid(row=8, rowspan=3, pady=10, sticky=tk.SW)
		# bottom frame 2/2
		self.frames[4].grid(row=8, column=1, rowspan=3, pady=10, sticky=tk.SE)

		for i in range(4):
			self.frames[i].rowconfigure(i, weight=1)
			self.frames[i].columnconfigure(i, weight=1)

		self.roundName.grid(row=0, column=0, sticky=tk.NW)
		self.questionNumber_label.grid(row=1, column=0, sticky=tk.SW)

		self.timer.start_button.grid(row=0, column=1, padx=2, pady=5, sticky=tk.E)
		self.timer.stop_button.grid(row=0, column=2,  padx=2, pady=5, sticky=tk.E)
		self.timer.label.grid(row=0, column=6, rowspan=2, columnspan=2, padx=20, sticky=tk.E)
		...
		## bottom frame ##
		self.menuButton.grid(row=0, column=0, sticky=tk.W)
		self.undo_button.grid(row=0, column=0, sticky=tk.E)
		self.redo_button.grid(row=0, column=1, sticky=tk.E)
		self.circleButton7.grid(row=0, column=2, sticky=tk.E)
	...
	def redo(self):
		if (self.history_ptr == -1):
			...
			recent_action[0]._plus(recent_action[1])
			_debug("Redo")
		elif (self.history_ptr+1 < len(self.history)):
			recent_action = self.history[self.history_ptr+1]
			recent_action[0]._plus(recent_action[1])
			_debug("Redo")
			self.history_ptr += 1
...
class GamePauseScreen(tk.Frame):
	def __init__(self, control, master=None):
		...
		self.editContestants = tk.Button(self, text='Edit Contestants', command=lambda:self.control.loadNavi(0), font=self.button_font, bg=light, fg=light_txt)
		self.testBuzzers = tk.Button(self, text='Test Buzzers',font=self.button_font, command=lambda:self.control.loadNavi(1), bg=light, fg=light_txt)
		self.activityLog = tk.Button(self, text='Activity Log', font=self.button_font, command=lambda:self.control.loadNavi(2), bg=light, fg=light_txt)
